...
import { resolveCodeValue, type BarcodeSymbology } from '../../../lib/codes';
...
interface ProductLabelProps {
  ...
  /** Precio ya formateado por el padre, para no montar el hook de moneda por etiqueta. */
  priceLabel?: string;
  /** Guía de corte en pantalla. No se imprime. */
  showGuide?: boolean;
}
...
export default function ProductLabel({ product, options, priceLabel, showGuide }: ProductLabelProps) {
  ...
  return (
    <div
      className={showGuide ? 'border border-dashed border-gray-300' : ''}
      style={{
    ...
    >
      ...
      >
        {isQr ? (
          <QRCodeSVG value={value} size={qrSize} margin={2} />
        ) : (
          <Barcode1D
            value={value}
            format={options.symbology === 'EAN13' ? 'EAN13' : 'CODE128'}
            height={barcodeHeight}
            moduleWidth={1.4}
            displayValue={!options.showSku}
            fontSize={11}
            margin={4}
          />
        )}
      ...
      {options.showSku && (
        <div style={{ fontSize: '2.4mm', fontFamily: 'monospace', lineHeight: 1.1, flexShrink: 0 }}>{value}</div>
      )}

      {options.showPrice && priceLabel && (
        <div style={{ fontSize: '3mm', fontWeight: 700, lineHeight: 1.15, flexShrink: 0 }}>{priceLabel}</div>
      )}
    </div>
  );
}
