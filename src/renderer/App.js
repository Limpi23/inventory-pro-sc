    ...
    useEffect(() => {
        ...
        const handleCheckMigrationStatus = async () => {
            ...
            try {
                ...
                if (faltantes.length) {
                    toast.error(`La base de datos no está actualizada (falta: ${faltantes.join(', ')}). ` +
                        'Pide al administrador que aplique las migraciones.', { id: 'check-db' });
                }
                else {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (faltantes.length) {
                    const { toast } = await import('react-hot-toast');
                    toast.error(`La base de datos no está actualizada para esta versión (falta: ${faltantes.join(', ')}). ` +
                        'Pide al administrador que aplique las migraciones.', { id: 'schema-outdated', duration: Infinity });
                }
            }
            catch {
