...
export function unsupportedCode128Chars(value) {
    const bad = new Set();
 ...
 */
export function resolveCodeValue(product) {
...
export function validateCode(value, type) {
    const v = (value || '').trim();
    if (!v) {
 ...
 */
export function effectiveSymbology(product) {
    const t = product?.barcode_type;
    if (t === 'CODE128' || t === 'EAN13' || t === 'QR')
        return t;
